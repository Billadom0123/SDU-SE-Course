...
public class GradeStudentHomeworkCourseController {
    ...
    public List getAllCourseHomeworkList(Integer courseId, Integer gradeId){
        List dataList = new ArrayList();

        List<Homework> homeworkList = homeworkRepository.findHomeworkByCourseIdAndGradeId(courseId,gradeId);
        ...
        for(int i=0;i<homeworkList.size();i++){
            ...
            String gradeStudentHomeworkParas = "model=gradeStudentHomework&gradeId="+gradeId;
            m.put("gradeStudentHomework","返回");
    ...
    @PostMapping("/gradeStudentHomeworkCourseInit")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse gradeStudentHomeworkCourseInit(@Valid @RequestBody DataRequest dataRequest) {
        courseId = dataRequest.getInteger("courseId");
        gradeId = dataRequest.getInteger("gradeId");
        List dataList = getAllCourseHomeworkList(courseId,gradeId);
        return CommonMethod.getReturnData(dataList);
    }

    @PostMapping("/gradeStudentHomeworkCourseEditInit")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse gradeStudentHomeworkCourseEditInit(@Valid @RequestBody DataRequest dataRequest){
    ...
    @PostMapping("/gradeStudentHomeworkCourseEditSubmit")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse gradeStudentHomeworkCourseEditSubmit(@Valid @RequestBody DataRequest dataRequest){
    ...
    @PostMapping("/gradeStudentHomeworkCourseDelete")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse gradeStudentHomeworkCourseDelete(@Valid @RequestBody DataRequest dataRequest){
        Integer id = dataRequest.getInteger("id");
        Homework A = null;
        Optional<Homework> op;
