...
public class GradeStudentAchievementCourseController {
    ...
    public List getAllCourseAchievementList(Integer courseId, Integer gradeId){
        List dataList = new ArrayList();

        List<Achievement> achievementList = achievementRepository.findAchievementByCourseIdAndGradeId(courseId,gradeId);
        ...
        for(int i=0;i<achievementList.size();i++){
            ...
            String gradeStudentAchievementParas = "model=gradeStudentAchievement&gradeId="+gradeId;
            m.put("gradeStudentAchievement","返回");
    ...
    @PostMapping("/gradeStudentAchievementCourseInit")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse gradeStudentAchievementCourseInit(@Valid @RequestBody DataRequest dataRequest) {
        courseId = dataRequest.getInteger("courseId");
        gradeId = dataRequest.getInteger("gradeId");
        List dataList = getAllCourseAchievementList(courseId,gradeId);
        return CommonMethod.getReturnData(dataList);
    }

    @PostMapping("/gradeStudentAchievementCourseEditInit")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse gradeStudentAchievementCourseEditInit(@Valid @RequestBody DataRequest dataRequest){
    ...
    @PostMapping("/gradeStudentAchievementCourseEditSubmit")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse gradeStudentAchievementCourseEditSubmit(@Valid @RequestBody DataRequest dataRequest){
    ...
    @PostMapping("/gradeStudentAchievementCourseDelete")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse gradeStudentAchievementCourseDelete(@Valid @RequestBody DataRequest dataRequest){
        Integer id = dataRequest.getInteger("id");
        Achievement A = null;
        Optional<Achievement> op;
