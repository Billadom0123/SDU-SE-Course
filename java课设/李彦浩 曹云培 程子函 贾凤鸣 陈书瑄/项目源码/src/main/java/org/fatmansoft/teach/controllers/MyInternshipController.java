...
public class MyInternshipController {
    ...
    public List getMyPracticeListByStudentId(Integer Id){
        ...
        for(MyInnovation myInnovation:myInnovationList){
            ...
            if(myInnovation.getInternship()!=null){
                m.put("internship",myInnovation.getInternship());
            }
            else{
                continue;
            }
    ...
    @PostMapping("/myInternshipInit")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse myInternshipInit(@Valid @RequestBody DataRequest dataRequest){
        studentId = dataRequest.getInteger("studentId");
        List dataList = getMyPracticeListByStudentId(studentId);
        return CommonMethod.getReturnData(dataList);
    }

    @PostMapping("/myInternshipEditInit")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse myInternshipEditInit(@Valid @RequestBody DataRequest dataRequest) {
        ...
        if(s != null) {
            form.put("id",s.getId());
            form.put("internship", s.getInternship());
        }
        return CommonMethod.getReturnData(form);
    }

    @PostMapping("/myInternshipEditSubmit")
    @PreAuthorize(" hasRole('ADMIN')")
    public DataResponse myInternshipEditSubmit(@Valid @RequestBody DataRequest dataRequest) {
        Map form = dataRequest.getMap("form");
        Integer id = CommonMethod.getInteger(form,"id");
        String practice = CommonMethod.getString(form,"internship");
        MyInnovation s= null;
        Optional<MyInnovation> op;
        if(id != null) {
            op = myInnovationRepository.findById(id);
        ...
        s.setInternship(practice);
        myInnovationRepository.save(s);
        return CommonMethod.getReturnData(s.getId());
    }

    @PostMapping("/myInternshipDelete")
    @PreAuthorize(" hasRole('ADMIN')")
    public DataResponse myInternshipDelete(@Valid @RequestBody DataRequest dataRequest) {
