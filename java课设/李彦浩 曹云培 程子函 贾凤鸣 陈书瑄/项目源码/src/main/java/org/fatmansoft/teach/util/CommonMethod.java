...
public class CommonMethod {
    ...
    //getReturnData的重载方法，参数列表里只有一个对象
    public static  DataResponse getReturnData(Object obj){
        /*
        在调用getReturnData(obj,null)时,会创建一个映射集data
        第一个键值对 <"data" obj>
        第二个键值对 <"msg" null>
        随后返回一个DResponse类型对象 第一个参数:"0" 第二个参数:data
        return new DataResponse("0",{<"data" obj>
                                     <"msg" null>})
        */
    ...
    public static Student getStudent(Map data,String key){
        Object obj = data.get(key);
        if(obj == null)
            return null;
        if(obj instanceof Student)
            ...
            return new Student();//返回新的学生对象，会不会没有这个学生但创建了一个新的学生
    }
