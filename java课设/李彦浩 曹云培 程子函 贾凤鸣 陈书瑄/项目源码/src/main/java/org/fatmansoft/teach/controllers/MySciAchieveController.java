...
public class MySciAchieveController {
    ...
    public List getMyCompetitionListByStudentId(Integer Id){
        ...
        for(MyInnovation myInnovation:myInnovationList){
            ...
            if(myInnovation.getSciAchieve()!=null){
                m.put("sciAchieve",myInnovation.getSciAchieve());
            }
            else{
                continue;
            }
    ...
    @PostMapping("/mySciAchieveInit")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse mySciAchieveInit(@Valid @RequestBody DataRequest dataRequest){
        studentId = dataRequest.getInteger("studentId");
        List dataList = getMyCompetitionListByStudentId(studentId);
        return CommonMethod.getReturnData(dataList);
    }

    @PostMapping("/mySciAchieveEditInit")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse mySciAchieveEditInit(@Valid @RequestBody DataRequest dataRequest) {
        ...
        if(s != null) {
            form.put("id",s.getId());
            form.put("sciAchieve", s.getSciAchieve());
        }
        return CommonMethod.getReturnData(form);
    }

    @PostMapping("/mySciAchieveEditSubmit")
    @PreAuthorize(" hasRole('ADMIN')")
    public DataResponse mySciAchieveEditSubmit(@Valid @RequestBody DataRequest dataRequest) {
        Map form = dataRequest.getMap("form");
        Integer id = CommonMethod.getInteger(form,"id");
        String practice = CommonMethod.getString(form,"sciAchieve");
        MyInnovation s= null;
        Optional<MyInnovation> op;
        if(id != null) {
            op = myInnovationRepository.findById(id);
        ...
        s.setSciAchieve(practice);
        myInnovationRepository.save(s);
        return CommonMethod.getReturnData(s.getId());
    }

    @PostMapping("/mySciAchieveDelete")
    @PreAuthorize(" hasRole('ADMIN')")
    public DataResponse mySciAchieveDelete(@Valid @RequestBody DataRequest dataRequest) {
